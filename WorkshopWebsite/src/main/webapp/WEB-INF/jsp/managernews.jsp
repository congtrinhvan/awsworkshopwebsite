                  ...
                  <div class="tab-content">
                    <div role="tabpanel" class="tab-pane active" id="creatnews">
                     ...
                     <div class="formtopic">
                        ...
                        <div class="row space-fiels">
                          <div class="col-md-3 col-sm-3 col-xs-12 alinetexxt">
                            <label for="newstitle">Tóm tắt nọi dung </label><span class="require">*</span>
                          </div>
                          <div class="col-md-9 col-sm-9 col-xs-12">
                            <textarea rows="3" class="form-control" id="newssumary" required=""></textarea>
                          </div>
                        </div>
                        <div class="clearfix"></div>
                      ...
                      <table class="table table-bordered custable customtable2 table-hover" id="tablestatistics1">
                        <thead>
                          <tr>
                            ...
                            <th>ID</th>
                            <th>Tiêu đề</th>
                            <th style="    width: 33%;">Tóm tắt</th>
                            <th style=" width: 22%;">Ngày tin tức</th>
                            <th></th>
                          </tr>
                        </thead>
                        <tbody id="myTable">
                     ...
                     <div class="row space-fiels">
                        ...
                        <div class="clearfix"></div>
                         ...
                         <div class="row space-fiels">
                          ...
                          <div class="col-md-9 col-sm-9 col-xs-12">
                            <textarea rows="3" class="form-control" id="modynewssumary" required=""></textarea>
                          </div>
                        </div>
                        <div class="clearfix"></div>
                                    ...
                                    <div class="space-fileup" style="margin-top: 0px;">
                                      <label class="input-label" for="file2">
                                        <i class="fa fa-upload"></i>
                                          ...
                                          <input type="file" name="file" id="file2" accept=".pdf" value="" style="display:none;">
...
<!-- Custom styling plus plugins -->
  ...
  <script type="text/javascript">
                ...
                for(var i = 0 ; i<result.length; i++){
                	if(temp==0){
                  	  html+="<tr class='cus-colo'>"+
  	            	  	...
  	            	  	"<td>"+result[i].news_sumary+"</td>"+
  	            	  		"<td>"+result[i].news_date+"</td>"+
                                    ...
                                    type: "POST",
                                        ...
                                        success: function (result){
                                          if(result!=null){
                                            ...
                                            $("#modynewssumary").val(result.news_sumary);
                                            CKEDITOR.instances.modinewscontent.setData(result.news_content)
                                            $("#modinewscontent").val(result.id)
                                            $("#modiimgnewsup").attr("src",result.news_image);
                                            $("#readfiles").attr("src",result.news_file);
    ...
    function UpdateNews(){
		...
		var id = $.trim($("#modiIDnews").val());
    	var title = $.trim($("#modinewstitle").val());
		var sumary = $.trim($("#modynewssumary").val());
		var content = $.trim(CKEDITOR.instances.modinewscontent.getData());
		var image = $.trim($("#modiFileImageNews").val());
		var file = $.trim($("#modiFileName").val());
		if(title==""||sumary==""||content==""){
			...
			if(image==""||file==""){
				$.confirm({
					title: 'Thông Báo',
					content: 'Tin tức không có hình ảnh hoặc tệp đính kèm!. Bạn chắc chắn không?',
					buttons: {
						Confirm : function(){
							$.ajax({
								url : 'updatenews',
								...
								success:function(result){
									if(result==200){
										...
										$.alert("Tạo mới tin tức không thành công!");
									}
								},error : function(){
									$.alert("Tạo mới tin tức không thành công!")
								}
    ...
    function UploadFile(){
		...
		if(!file){
			...
			$.ajax({
				...
				contentType : false,
				success:function(result){
					$("#FileNamess").html(result[1]);
 					$("#FileName").val(result[0]);
 					$("#reviewfiles").attr({'src':result[0]});
				},error:function(error, status){
				...
				$.ajax({
					...
					success:function(result){
						$("#modiimgnewsup").attr({'src':result[0]});
	 					$("#modiFileImageNews").val(result[0])
					},error:function(error, status){
						$.alert("Tải ảnh lên không thành công");
					}
				});
    ...
    function UploadFile2(){
		...
		if(!file){
			...
			$.ajax({
				...
				contentType : false,
				success:function(result){
					$("#modiFileNamess").html(result[0]);
 					$("#modiFileName").val(result[0]);
				},error:function(error, status){
					$.alert("Tải tệp lên không thành công");
				}
			});
	...
	function CreateNews(){
		...
		var sumary = $.trim($("#newssumary").val());
		var content = $.trim(CKEDITOR.instances.newscontent.getData());
		var image = $.trim($("#FileImageNews").val());
		var file = $.trim($("#FileName").val());
		if(title==""||sumary==""||content==""){
			$.alert("Vui lòng nhập đầy đủ thông tin!");
		}else{
			if(image==""||file==""){
